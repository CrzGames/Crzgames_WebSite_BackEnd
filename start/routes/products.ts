...
router
  .group((): void => {
    router.post('/products', [controllers.Product, 'createProduct']).use(
      middleware.authRole([UserRoles.STAFF, UserRoles.ADMIN])
    )
    ...
    router.put('/products/:id', [controllers.Product, 'updateProduct']).use(
      middleware.authRole([UserRoles.STAFF, UserRoles.ADMIN])
    )
    router.delete('/products/:id', [controllers.Product, 'deleteProduct']).use(
      middleware.authRole([UserRoles.STAFF, UserRoles.ADMIN])
    )
