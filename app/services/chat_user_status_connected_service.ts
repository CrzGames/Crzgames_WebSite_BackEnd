import ChatUserStatusConnected from '#models/chat_user_status_connected'
...
export default class ChatUserStatusConnectedService {
  public static async updateUserStatusConnectedStatus(
    ...
    let chatUserStatusConnected: ChatUserStatusConnected | null = await ChatUserStatusConnected.findBy(
      'users_id',
      userId,
    )
  ...
  public static async getUserStatus(userId: number): Promise<string> {
    const userStatus: ChatUserStatusConnected | null = await ChatUserStatusConnected.findBy('users_id', userId)
