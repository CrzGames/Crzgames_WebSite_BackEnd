...
export default class AuthService {
  ...
  public static async forgotPassword(email: string): Promise<void> {
    ...
    await MailService.sendMail(
      ...
      {
        username: user.username,
        redirect_uri: env.get('APP_BASE_URL') + env.get('APP_REDIRECT_URI_FORGOT_PASSWORD') + passwordResetToken.token,
      },
  ...
  public static async resetPassword(token: string, password: string): Promise<void> {
    const passwordResetToken: PasswordResetToken = await PasswordResetTokenService.getPasswordResetTokenByToken(token)
  ...
  public static async sendMailToModifyEmail(oldEmail: string): Promise<void> {
    ...
    await MailService.sendMail(
      ...
      'modify-email',
      {
        username: user.username,
        redirect_uri:
          env.get('APP_BASE_URL') + env.get('APP_REDIRECT_URI_SEND_MAIL_TO_MODIFY_EMAIL') + passwordResetToken.token,
      },
