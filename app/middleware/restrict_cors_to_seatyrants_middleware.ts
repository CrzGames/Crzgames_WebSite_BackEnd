...
/**
 * Middleware pour permettre de restreindre les requetes HTTP uniquement
 * si elles proviennent du backend SeaTyrants.
 * Ce middleware verifie la cle API dans l'en-tete de la requete (X-API-KEY).
 */
export default class RestrictCorsToSeaTyrantsMiddleware {
  /**
   * Gere la requete entrante et verifie la cle API.
  ...
  public async handle({ request, response }: HttpContext, next: () => Promise<void>): Promise<void> {
    ...
    if (apiKey !== apiKeySecret) {
      response.unauthorized({ message: 'Unauthorized request. Invalid API key.' })
      return
    }
